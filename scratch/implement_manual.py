...
class StudentImportService:
    ...
    def manual_execute_import(school_id: str, project_id: str, valid_records: List[Dict[str, Any]], action: str) -> Dict[str, int]:
        ...
        for rec in valid_records:
            ...
            for f in ["dob", "address", "contact", "standard", "section", "roll_number", "gender"]:
                if f in rec:
                    base_doc[f] = rec[f]
                    
            if "custom_fields" in rec:
                base_doc["custom_fields"] = rec["custom_fields"]
                
            operations.append(base_doc)
        ...
        if action == "append":
            for op in operations:
                op["gr"] = rec["gr"] # Add gr back
                op["_id"] = ObjectId()
                op["photo_status"] = "not_captured"
                db.students.insert_one(op)
                inserted += 1
        elif action in ["replace", "update"]:
            # Find existing students to update
            # Since this is manual, let's keep it simple
            for op in operations:
                gr = rec["gr"] # wait, the loop variable was shadowed
                pass # I need to fix this variable capture in Python
